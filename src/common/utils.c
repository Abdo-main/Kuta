...
bool isFull(Stack *stack) { return stack->top == MAX_ENTITIES - 1; }
...
uint32_t pop(Stack *stack) {
  ...
  uint32_t popped = stack->arr[stack->top];
  stack->top--;
  return popped;
}
...
void end_single_time_commands(VkCommandBuffer command_buffer, State *state) {
  ...
  vkFreeCommandBuffers(state->vk_core.device, state->renderer.command_pool, 1,
                       &command_buffer);
}
...
VkImageView create_image_view(VkImage image, VkFormat format,
  ...
  EXPECT(vkCreateImageView(state->vk_core.device, &view_info,
                           state->vk_core.allocator, &image_view),
...
void exit_callback() { glfwTerminate(); }
...
void log_info() {
  ...
  EXPECT(vkEnumerateInstanceVersion(&instance_api_version),
  ...
  printf("Vulkan Api %i.%i.%i.%i\n", api_version_variant, api_version_major,
