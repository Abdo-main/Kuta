...
void create_descriptor_pool(State *state, Models *models) {
    ...
    pool_sizes[0].descriptorCount = total_sets;  // Changed from MAX_FRAMES_IN_FLIGHT
    pool_sizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    pool_sizes[1].descriptorCount = total_sets;  // Changed from MAX_FRAMES_IN_FLIGHT
    ...
    VkDescriptorPoolCreateInfo pool_info = {
        ...
        .maxSets = total_sets,  // Changed from MAX_FRAMES_IN_FLIGHT
    };
...
void create_descriptor_sets(BufferData *buffer_data, Models *models, State *state) {
    ...
    state->renderer.descriptor_sets = malloc(total_sets * sizeof(VkDescriptorSet));
    state->renderer.descriptor_set_count = total_sets;
    
    EXPECT(vkAllocateDescriptorSets(state->vk_core.device, &alloc_info, state->renderer.descriptor_sets), "Failed to allocate descriptor sets");
    ...
    for (size_t frame = 0; frame < MAX_FRAMES_IN_FLIGHT; frame++) {
        for (size_t model = 0; model < models->model_count; model++) {
            ...
            VkDescriptorImageInfo image_info = {
                ...
                .imageView = models->texture[model].texture_image_view,  // Use model index
            ...
            vkUpdateDescriptorSets(state->vk_core.device, 2, descriptor_writes, 0, NULL);
        }
    }
