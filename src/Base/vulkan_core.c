...
void create_instance(State *state) {
    ...
    EXPECT(vkCreateInstance(&createInfo, state->allocator, &state->instance), "Failed to Create vulkan Instance");
}
...
void select_physical_device(State *state) {
    ...
    EXPECT(vkEnumeratePhysicalDevices(state->instance, &count, NULL), "Failed to enumerate physical devices count1")
    EXPECT(count == 0, "Failed to find vulkan supported physical device")

    VkPhysicalDevice devices[count];
    VkResult result = vkEnumeratePhysicalDevices(state->instance, &count, devices);
...
void create_surface(State *state) {
    EXPECT(glfwCreateWindowSurface(state->instance, state->window, state->allocator, &state->surface), "Failed to create window surface")
}
...
void select_queue_family(State *state) {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(state->physical_device, &count, NULL);
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(state->physical_device, &count, queue_families);
    ...
    for (uint32_t queue_family_index = 0; queue_family_index < count; ++queue_family_index) {
        ...
        if(properties.queueFlags & VK_QUEUE_GRAPHICS_BIT && glfwGetPhysicalDevicePresentationSupport(state->instance, state->physical_device, queue_family_index)){
            state->queue_family = queue_family_index;
            break;
        }
    }
    EXPECT(state->queue_family == UINT32_MAX, "Failed no suitable queue family")
