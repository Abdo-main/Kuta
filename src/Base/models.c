...
void load_models(const char* filenames[], Models *models, size_t count, BufferData *buffer_data, State *state) {
    ...
    for(size_t i = 0; i < count; i++){
        ...
        const struct aiScene* scene = aiImportFile(filenames[i], 
    ...
    for(size_t i = 0; i < count; i++){
        ...
        models->geometry[i].vertices = malloc(sizeof(Vertex) * total_vertices);
        models->geometry[i].indices = malloc(sizeof(uint32_t) * total_indices);
        ...
        for (unsigned int m = 0; m < scene->mNumMeshes; m++) {
            struct aiMesh* mesh = scene->mMeshes[m];
            size_t vertex_offset = models->geometry[i].vertex_count; // Use the actual counter
            ...
            for (unsigned int v = 0; v < mesh->mNumVertices; v++) {
                ...
                models->geometry[i].vertices[models->geometry[i].vertex_count++] = vertex;
            }
        ...
        printf("  - Vertex reuse ratio: %.1f:1\n", 
               (float)models->geometry[i].index_count / models->geometry[i].vertex_count);
